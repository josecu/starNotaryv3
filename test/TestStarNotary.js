...
it('lets user1 put up their star for sale', async() => {
    ...
    assert.equal(await instance.starOwner.call(starId), user1);
...
it('lets 2 users exchange stars', async() => {
    ...
    assert.equal(await instance.starOwner.call(starId1), user1);
    assert.equal(await instance.starOwner.call(starId2), user2);
    // 2. Call the exchangeStars functions implemented in the Smart Contract
    await instance.exchangeStars(starId1, starId2, {from: user1});
    // 3. Verify that the owners changed
    assert.equal(await instance.starOwner.call(starId1), user2);
    assert.equal(await instance.starOwner.call(starId2), user1);
});
...
it('lets a user transfer a star', async() => {
    ...
    assert.equal(await instance.starOwner.call(starId), user2);
});
